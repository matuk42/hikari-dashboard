...
> **ZRUŠENO (rozhodnutí 2026-06-23):** Tento workflow se **nebude dělat**. Hlasové deníky půjdou **vždy jen do vaultu (2nd_brain)** — nikdy se nezapisují zpět do Supabase. Habit status žije výhradně v dashboardu (klik = pravda), deník je čistá reflexe, která se ho nedotýká. Konflikt „klik ✅ vs. deník nevyšlo" tedy nemá jak vzniknout. Sekce ponechána jen jako záznam zvažované a zamítnuté varianty.
...
- [x] Auto-retire habits — **hotovo 23.6**: `autoRetireHabits` jako krok 0 v `runMorningCron` (ranní cron + tlačítko). `end_date < dnes` → `category='retired'` + `retired_on` + důvod. Jen `end_date` (ne `trial_end` — to byl zrušený package-eval flow). Ověřovák `scripts/check-autoretire.mjs`.
...
- [~] **Cascade datové kotvy pro reálná %** (návazné na 22.6) — **(1) aktuální příjem ✅ hotovo 23.6**: migrace 009 `income_snapshots` (append-only) + `IncomeAnchorCard` na `/cascade` (měsíčně/hodinovka/celkem) + napojení do `calcMilestonePct` promptu (sekce „AKTUÁLNÍ PŘÍJEM"; bez snapshotu Gemini drží příjem ~0, nehádá). Ověřovák `scripts/check-income.mjs`. **Zbývá:** (2) fyzička čísla (shyby/běh/kolo z [[2027]] „měřitelnost"), (3) JLPT úroveň / počet Anki karet — stejným způsobem. Volitelně: tvrdý clamp kadencových týdenních milníků + „potvrdit milník" UI.
