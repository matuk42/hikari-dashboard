...
- **Cascade** — týden (L5) a měsíc (L4) **počítá Hikari z habitů** (reálná % z DB). **Milníková % L3/L4/L5 + layer-% L2/L3 odhaduje Gemini** on-demand přes „Přepočítej Hikari" (17.6. session 4) — `VaultDimRow` ukazuje % + bar když >0. Milníky se čtou živě z DB (z vaultu), rolují se na nový měsíc/týden/rok při syncu. L1 chips + L2 dimenze zůstávají kurátované (L2 jen layer-%).
