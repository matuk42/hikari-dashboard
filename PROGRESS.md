...
**Detekce vzorů → proposed memory + schvalovací UI** (hybrid stat+Gemini, session 8). Chybí: auto-retire habits, /calculator. **Konflikt workflow (CLI ↔ Supabase) přesunut z W26 na V3** (rozhodnutí 23.6.) — nemá jak vzniknout, dokud voice deník přes CLI nezapisuje habity/HOPE zpět do Supabase; design zůstává v PRD §3.5. (Auto-sync cron ✅ 22.6. — zřetězen do ranního cronu.) (Hodnotící zprávy balíčků — zrušeno 18.6.)
