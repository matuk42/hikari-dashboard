...
- **kibou** — slidery mood/energy/hope s vlastními PNG ikonkami. **Intraday check-iny** (24.6.): „Zaznamenat teď" = append check-in do `hope_checkins` (víckrát/den), denní číslo = průměr (rollup do `hope_logs`). **Křivka dne** (osa X = čas) + day stepper + sekce „Co ti hýbe energií" (korelace). 30d trend graf z rollupu. ⚠️ vyžaduje migraci 010.
- **Živá energetická osa** — home čte `energy_blocks` pro dnešní day_of_week. **Učí se reálný tvar** (24.6.): bloky s ≥3 intraday check-iny berou naměřený průměr, jinak fallback na syntetický `BASE_CURVE` × scale. Aktivní blok: zlatý bod + scaleY + glow. Fallback na statické hodnoty před prvním cronem.
...
**Migrace:** 003–008 aplikované. **009 `income_snapshots` ČEKÁ na spuštění** (kotva příjmu pro cascade %). **010 `hope_checkins` ČEKÁ na spuštění** (intraday HOPE check-iny — oblouky dne + korelace). 008 = `ai_daily_brief.speaking JSONB`.
