  ...
  return (
    ...
    <img
      ...
      style={{
        ...
        pointerEvents: 'none', userSelect: 'none', zIndex: 0, opacity: 0.05,
      }}
    />
  )
}
...
function SummaryStat({ value, label, sub }: { value: number; label: string; sub?: string }) {
  return (
    <div style={{ position: 'relative', flex: 1, background: '#0e0e0e', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '12px 8px', textAlign: 'center' }}>
      ...
      {sub && <div style={{ position: 'absolute', left: 0, right: 0, bottom: 4, fontSize: 8, color: 'rgba(255,255,255,0.18)', lineHeight: 1 }}>{sub}</div>}
    </div>
  )
}
