...
interface Habit {
  ...
  frequency: string
  streak: number
  endDate?: string      // display form ("30.6.")
    ...
    <div style={{ background: '#0e0e0e', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, overflow: 'hidden' }}>
      ...
      {open && (
        <div style={{ padding: '0 16px 8px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          {habits.map(h => (
            <HabitRow key={h.id} habit={h} state={done.has(h.id) ? 'done' : rest.has(h.id) ? 'rest' : 'none'} onToggle={() => onToggle(h.id)} liveStreak={streakMap[h.id]} editMode={editMode} onEdit={() => onEdit?.(h)} />
          ))}
        </div>
      )}
    </div>
...
export default function HabitsPage() {
  ...
  const reloadHabits = useCallback(async () => {
    ...
    loadPackMeta(profileId).then(setPackMeta).catch(() => {})
  ...
  // the same rest-aware denominator so every surface agrees.
